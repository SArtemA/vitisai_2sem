...
После этого установка будет проще:

```bash
pip install -r requirements.txt
```

### 2. Вынести настройки в `.env`

Можно добавить файл `.env`:

```env
APP_HOST=127.0.0.1
APP_PORT=5459
DATABASE_URL=sqlite:///./vineyards_v2.db
GEE_PROJECT_ID=pp-2-sem-grapes
MODEL_PATH=xgboost_grape_model.json
```
...
- тест API `/api/predict`;
- тест функции `fetch_environmental_data`;
...
```bash
docker build -t vitipredict .
docker run -p 5459:5459 vitipredict
```

### 7. Добавить визуализацию результата

Например:

- цветовая индикация пригодности;
- отображение экологических признаков на карточке;
- слой пригодности на карте;
